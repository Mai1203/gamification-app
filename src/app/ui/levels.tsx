...
import AnimationLoaded from "./dashboard/animationLoaded";

interface LevelsProps {
  mode: string;
}

export default function Levels({ mode }: LevelsProps) {
  ...
  const htmlModule = isSignedIn 
    ? modules.find((mod) => mod.id === mode)
    : initialModules.find((mod) => mod.id === mode);
  ...
  const handleLessonClick = (index: number) => {
    ...
    router.push(`/learning?module=${mode}&level=${index + 1}`);
  };
